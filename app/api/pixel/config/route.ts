...
export async function GET(request: Request) {
    ...
    if (!clientId) {
        return NextResponse.json({ error: 'client_id is required' }, { status: 400, headers: corsHeaders });
    }
    ...
    try {
        ...
        if (error || !data) {
            return NextResponse.json({ success: false, error: 'Client not found' }, { status: 404, headers: corsHeaders });
        }
