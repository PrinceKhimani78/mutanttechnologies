...
export const Navbar = ({
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      const { data } = await import("@/lib/cms").then((mod) =>
        mod.getSiteSettings(),
      );
        ...
        <div className="h-full flex flex-col justify-center px-8 sm:px-12 pt-32">
          ...
          <div className="mobile-nav-footer mt-auto pb-12 border-t border-gray-200 dark:border-zinc-800 pt-8 flex flex-col sm:flex-row gap-6 items-start sm:items-center justify-between">
            <div className="flex flex-col gap-2">
              <span className="text-sm text-gray-500 dark:text-zinc-500 uppercase tracking-widest font-mono">
                Get in touch
              </span>
              <a
            ...
            <Button
              href="/contact"
              withIcon={true}
              className="w-auto text-sm sm:text-base px-6 py-3"
            >
              Start Project
            </Button>
          </div>
