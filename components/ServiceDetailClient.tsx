...
export default function ServiceDetailClient({
  ...
  useGSAP(
    () => {
      ...
      if (scroller) {
        console.log(
          "[ServiceDetailClient] Skipping GSAP animations in Visual Editor",
        );
        return;
      }
  ...
  return (
    <main
      ref={containerRef}
      className={cn(
        "bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 font-sans selection:bg-orange-500 selection:text-white transition-colors duration-300",
    ...
    >
      ...
      <section
        className={cn(
          ...
          scroller
            ? "items-start min-h-[400px] pt-20 pb-12"
            : "items-center min-h-[90vh] pt-24 pb-12 lg:pt-32 lg:pb-20",
        )}
      >
        {/* Background Glows */}
        <div
          className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] lg:w-[800px] h-[400px] lg:h-[800px] rounded-full blur-[80px] lg:blur-[120px] pointer-events-none opacity-20 bg-linear-to-br ${service.bg_gradient || "from-blue-600 to-purple-600"}`}
        />
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
            <div className="lg:w-[60%] hero-content text-center lg:text-left">
              ...
              <div className="flex flex-col sm:flex-row flex-wrap gap-4 mt-8 lg:mt-10 justify-center lg:justify-start">
                <Button
                  size="lg"
                  className="bg-orange-600 hover:bg-orange-700 text-white rounded-full h-14 px-10 text-lg transition-transform hover:scale-105 w-full sm:w-auto whitespace-nowrap"
                  onClick={() =>
                    document
                ...
                <Button
                  size="lg"
                  variant="outline"
                  className="border-zinc-700 hover:bg-white/10 text-white rounded-full h-14 px-10 text-lg flex items-center justify-center gap-2 w-full sm:w-auto whitespace-nowrap"
                  onClick={() =>
                    document
                ...
                >
                  <span className="flex items-center gap-3">
                    <span className="w-5 h-5 border border-current rounded-sm flex items-center justify-center text-[10px]">
                      P
                    </span>
                    <span className="whitespace-nowrap">View Process</span>
                  </span>
                </Button>
              </div>
            </div>
                ...
                <div className="bg-zinc-900 rounded-[24px] lg:rounded-[32px] overflow-hidden border-4 lg:border-8 border-zinc-800 shadow-2xl aspect-[3/4] sm:aspect-[4/3] lg:aspect-[3/4]">
                  {service.hero_image ? (
                    ...
                    <div className="h-full w-full bg-zinc-950 p-8 flex flex-col relative overflow-hidden group items-center justify-center">
                      <div
                        className={`absolute inset-0 bg-linear-to-br ${service.bg_gradient} opacity-20`}
                      ></div>
                      <Icon className="w-32 h-32 text-zinc-700 relative z-10" />
                      <div className="mt-8 text-2xl font-bold text-zinc-500 z-10">
                        {service.title}
                      </div>
                      <div className="font-mono text-zinc-700 text-sm mt-2 z-10">
                        {service.id}
                      </div>
                    </div>
                  )}
      ...
      <section
        className={cn(
          "py-16 bg-white dark:bg-zinc-950 border-b border-zinc-100 dark:border-zinc-800",
          scroller && "py-8",
        )}
      >
        <p className="text-center text-zinc-500 dark:text-zinc-400 font-medium mb-10 tracking-widest text-sm">
        ...
        <div className="overflow-hidden relative w-full">
          <div className="marquee-content flex gap-16 whitespace-nowrap min-w-full items-center">
            {[...navLogos, ...navLogos, ...navLogos].map((logo, i) => (
              <span
                key={i}
                className="text-2xl font-bold text-zinc-300 dark:text-zinc-700 tracking-tighter"
              >
                {logo}
              </span>
            ))}
                ...
                <div className="bg-zinc-900 dark:bg-black rounded-3xl p-2 shadow-2xl rotate-2 hover:rotate-0 transition-transform duration-500">
                  <div className="aspect-video bg-zinc-800 rounded-2xl overflow-hidden relative flex items-center justify-center">
                    {service.benefits_image ? (
                      ...
                      <>
                        <div
                          className={`absolute inset-0 bg-linear-to-br ${service.bg_gradient} opacity-30`}
                        ></div>
                    ...
                    >
                      <div className="p-6 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <span
                            className={`font-mono text-sm ${activeAccordion === idx ? "text-orange-600" : "text-zinc-400 dark:text-zinc-500"}`}
                          >
                            0{idx + 1}
                          </span>
                        ...
                        <ChevronDown
                          className={`w-5 h-5 transition-transform ${activeAccordion === idx ? "rotate-180 text-orange-600" : "text-zinc-400 dark:text-zinc-500"}`}
                        />
                      </div>
                      <div
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {genericCaseStudies.map((study, idx) => (
              <div key={idx} className="group reveal-up cursor-pointer">
                ...
                <p className="text-zinc-500 dark:text-zinc-400 font-mono text-sm tracking-wider">
                  {study.category}
                </p>
              </div>
            ))}
          ...
          <div className="flex flex-col lg:flex-row gap-16 items-center">
            ...
            <div className="lg:w-1/2 w-full">
              <div className="bg-zinc-900 rounded-3xl p-8 border border-zinc-800 relative reveal-up">
                <div className="text-orange-500 text-6xl font-serif absolute top-4 right-8 opacity-20">
                  "
                </div>
                <p className="text-xl leading-relaxed text-zinc-300 mb-8 relative z-10">
                  "Mutant Technologies didn't just deliver a service; they
                  transformed our entire digital infrastructure. The speed of
                        ...
                        <div className="w-full h-full bg-white relative overflow-hidden rounded-2xl">
                          {featureImage ? (
                            ...
                            <div className="absolute inset-0 bg-gray-100 flex items-center justify-center text-zinc-300">
                              <div className="text-4xl font-heading opacity-20 text-center px-4">
                                {title}
                                <br />
                                Mockup
                              </div>
      ...
      <section id="process" className="py-24 bg-zinc-50 dark:bg-zinc-900/50">
        <div className={cn("container mx-auto px-6", scroller && "max-w-full")}>
          <h2 className="text-center text-4xl font-bold font-heading text-zinc-900 dark:text-white mb-20">
            Complete Lifecycle
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            {service.process?.map((item: any, idx: number) => (
              <div
                key={idx}
                className="bg-white dark:bg-zinc-900 p-8 rounded-2xl border border-zinc-100 dark:border-zinc-800 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group"
              >
                <div className="text-6xl font-bold text-orange-500 mb-6 transition-colors">
                  {item.step}
                </div>
      ...
      <section
        id="proposal-form"
        className="py-24 lg:py-32 px-6 relative overflow-hidden bg-gray-100 dark:bg-zinc-900"
      >
        <div
          className={`absolute top-0 left-0 w-full h-full bg-linear-to-b ${service.bg_gradient} opacity-5`}
        ></div>
        <div className="container mx-auto relative z-10 flex flex-col lg:flex-row gap-16 items-start">
          <div className="lg:w-1/2">
            ...
            <p className="text-xl text-gray-600 dark:text-zinc-400 font-light mb-12 max-w-md">
              Let's discuss your ideas and build something extraordinary
              together. Fill out the form, and we'll be in touch within 24
              hours.
            </p>
